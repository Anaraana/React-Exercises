    ...
    <div className="language-container">
      <div>
        <button onClick={() => setShowOtherFlags((curr) => !curr)}>
          <img className='small-language-container'src={selectedCountry.flagSrc}  alt="country flag" />
        </button>
      </div>
      {showOtherFlags && (
        <div>
          {countries
            ...
            .map((c, cIdx) => (
              ...
              >
                <img className='small-language-container2' src={c.flagSrc} alt="country flag" />
                <span>{c.country}</span>
              </button>
            ))}
